...
class SPR:
    ...
    def run(self):
        while True:
            if self.blind_game.get_answer_num() == 5:
                print "--------------------end----------------", self.blind_game.get_answer_num()
                break
            time.sleep(1)
    ...
    def count_num_of_human(self):
        ...
        if self.num_woman + self.num_man == 0:
            print"-----0-----"
            cmd = "sshpass -p kurakura326 scp ./nobody.png nao@" + str(self.ip) + ":~/.local/share/PackageManager/apps/boot-config/html"
            os.system(cmd)
            self.TabletSer.hideImage()
            self.TabletSer.showImageNoCache("http://198.18.0.1/apps/boot-config/nobody.png")
        else:
